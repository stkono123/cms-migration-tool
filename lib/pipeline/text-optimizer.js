...
const LEVEL_PROMPTS = {
  1: (text, tov) => `${tov}Korrigiere nur Rechtschreibung und Grammatik in folgendem Text. Ändere nichts am Inhalt, Stil oder der Struktur. Gib nur den korrigierten Text zurück, ohne Erklärungen:\n\n${text}`,
  2: (text, tov) => `${tov}Korrigiere Rechtschreibung und Grammatik und verbessere leicht die Formulierungen für bessere Lesbarkeit. Behalte den Inhalt und den Ton bei. Gib nur den verbesserten Text zurück, ohne Erklärungen:\n\n${text}`,
  3: (text, tov, persona) => `${tov}Korrigiere Rechtschreibung und Grammatik, verbessere die Formulierungen und passe den Ton und die Sprache an folgende Zielgruppe an: ${persona || 'allgemeine Verbraucher'}. Behalte alle inhaltlichen Informationen bei. Gib nur den angepassten Text zurück, ohne Erklärungen:\n\n${text}`,
  4: (text, tov, persona, keyword) => `${tov}Korrigiere Rechtschreibung und Grammatik, verbessere die Formulierungen, passe den Ton an folgende Zielgruppe an: ${persona || 'allgemeine Verbraucher'}, und optimiere den Text für SEO. ${keyword ? `Ziel-Keyword: "${keyword}". ` : 'Ermittle selbst das passendste Keyword. '}Integriere das Keyword natürlich. Gib nur den optimierten Text zurück, ohne Erklärungen:\n\n${text}`,
