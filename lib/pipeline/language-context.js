...
// Set deliberately low so that short but valid German phrases like
// "Willkommen auf unserer Website!" (31 chars) are not discarded.
 ...
 *
 * Strategy:
 *  1. Prefer items longer than MIN_MEANINGFUL_LENGTH (exclude IDs / brand names).
 *  2. If nothing passes that filter, fall back to joining all non-empty items —
 *     their combined length is usually enough for reliable detection.
 *
...
function buildSample(items) {
  ...
  const candidates = items.slice(0, MAX_SAMPLE_ITEMS).map(toText)

  // Primary: only use items long enough to carry language signal
  const meaningful = candidates.filter(t => t.trim().length >= MIN_MEANINGFUL_LENGTH)
  if (meaningful.length > 0) return meaningful.join('\n\n')

  // Fallback: concatenate whatever we have — short phrases combined are often
  // enough (e.g. several German column headers or short product names)
  return candidates.filter(t => t.trim()).join(' ').slice(0, 800)
}
